...
describe('edgeSilenceCuts / trimEdgeSilence — 앞뒤 무음만 제거(내부 쉼 불변)', () => {
  test('앞 400ms·뒤 500ms 무음 — 패드(앞40·뒤120)만 남기고 제거, 내부 쉼은 그대로', () => {
    const s = signal([
      { ms: 400, amp: 0 },
      { ms: 300, amp: 0.5 },
    ...
    expect(edgeSilenceCuts(s, SR)).toEqual([
      ...
      { start: 1420, end: 1800 }, // 뒤: 120ms 패드만 남김
    ]);
    const out = trimEdgeSilence(s, SR);
    expect(out.length).toBe(1800 - 360 - 380);
    // 말소리+내부 쉼 구간은 샘플 단위 무손상
    expect(out).toEqual(s.slice(360, 1420));
  });
