...
describe('어미 병합 — 닫는 따옴표 경계(codex 교차검증 지적)', () => {
  const { normalizeForSpeech } = require('../tts/sherpa/normalizeKo');
  test('어미 조각 뒤에 닫는 따옴표가 와도 병합(v1.27.1: 따옴표는 합성 입력에서 제거)', () => {
...
describe('말줄임 표기 통일(v1.27.0) — ASCII 점 연속은 이 팩이 "소리로 읽는다"', () => {
  ...
  test('…와 .의 혼합 연속(세로쓰기 소설 "…….")도 … 하나로(v1.27.1 — 잔존점 꼬리 잔향 실측)', () => {
    expect(normalizeForSpeech('보면…….')).toBe('보면…');
    expect(normalizeForSpeech('보면….')).toBe('보면…');
    expect(normalizeForSpeech('보면…..')).toBe('보면…');
    expect(normalizeForSpeech('보면..…')).toBe('보면…');
    expect(normalizeForSpeech('보면.…')).toBe('보면…'); // 마침표가 앞에 오는 꼴(교차검증 합의 반례)
    expect(normalizeForSpeech('보면.….')).toBe('보면…');
  });
});

describe('따옴표류 제거(v1.27.1) — 닫는 따옴표 꼬리 유성 잔향 실측(ell4_probe 2026-07-23)', () => {
  const { normalizeForSpeech } = require('../tts/sherpa/normalizeKo');
  test('대사 따옴표(겹·홑·낫표)는 합성 입력에서 벗긴다', () => {
    expect(normalizeForSpeech('“예!”')).toBe('예!');
    expect(normalizeForSpeech('"왔나?"')).toBe('왔나?');
    expect(normalizeForSpeech('「제목」과 ‘생각’과 『책』')).toBe('제목과 생각과 책');
  });
  test('사용자 보고 재현 문장: 따옴표 제거 + 말줄임 통일이 함께 적용', () => {
    expect(normalizeForSpeech('“친분을 쌓는 것을 보면…….”')).toBe('친분을 쌓는 것을 보면…');
  });
  test('ASCII 아포스트로피(영어 축약)는 남긴다', () => {
    expect(normalizeForSpeech("don't stop")).toBe("don't stop");
  });
  test('스마트 아포스트로피(don’t)는 ASCII 로 접어 보존, 홑따옴표 인용은 제거(교차검증 반례)', () => {
    expect(normalizeForSpeech('don’t stop')).toBe("don't stop");
    expect(normalizeForSpeech('그는 ‘설마’ 하고 생각했다.')).toBe('그는 설마 하고 생각했다.');
  });
  test('이중 꺾쇠 따옴표(〝〞)도 제거', () => {
    expect(normalizeForSpeech('〝정말 좋습니 다〞.')).toBe('정말 좋습니다.');
  });
