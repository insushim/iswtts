...
/** 합성 입력에서 벗기는 따옴표류(⓪-2 근거 주석 참조). 짧은 문장 판정 지름길
 *  (SherpaTtsEngine.leadPadOf)과 공유 — 세트가 갈리면 판정 문턱이 어긋난다. */
export const SPOKEN_QUOTES = /[“”"「」『』‘’〝〞]/g;
...
export function normalizeForSpeech(text: string): string {
  ...
  //    v1.27.1: "……."(말줄임 2개 + 마침표 — 세로쓰기 소설 관행) 이 구규칙에선 "…."로
  //    잔존 마침표를 남겼는데, 실측(ell4_probe 2026-07-23) 이 잔존점이 문장 꼬리에 저레벨
  //    유성 잔향(중레벨 240ms vs 순수 … 120ms — "드~" 웅얼거림 체감)을 만든다. …와 .이
  //    섞인 연속은 전부 … 하나로 접는다.
  let out = text
    .replace(/[‥⋯]/g, '…') // 두점 말줄임(‥)·수학 말줄임(⋯) → …
    .replace(/\.{2,}/g, '…') // .. ... .... → …
    .replace(/\.+(?=…)/g, '') // 말줄임 "앞"의 잔존 마침표(".…" — 교차검증 codex·Gemini 합의 반례)
    .replace(/…[.…]+/g, '…'); // "……"·"…."·"….." 등 혼합 연속 → … 하나

  // ⓪-2 따옴표류 제거(v1.27.1). 발음이 없는 기호인데 실측(ell4_probe 2026-07-23) 이 팩은
  //    닫는 따옴표가 붙은 문장 꼬리에 저레벨 유성 잔향을 만든다(같은 문장 ".”" 260ms vs
  //    "." 60ms — 2.5× 스트레치에서 "드~" 같은 웅얼거림으로 들린 것의 주범). 대사 세그먼트는
  //    따옴표를 포함한 원문 그대로 합성 입력에 들어오므로 여기서 벗긴다 — 하이라이트·분할은
  //    원문 기준이라 무영향. ASCII 아포스트로피(')는 영어 축약(don't)과 겹쳐 제외하고,
  //    영문자 사이의 스마트 아포스트로피(don’t)는 ASCII 로 접어 보존한다(교차검증 반례).
  out = out.replace(/([A-Za-z])’(?=[A-Za-z])/g, "$1'").replace(SPOKEN_QUOTES, '');
